...
class tx_mmforum_cronbase {
	...
	/**
	 * Debugging constant. Used for regular logging messages.
	 * @var int
	 */
	var $DEBUG_LOG			=  0;
	...
	/**
	 * holds the path to the mm_forum
	 * @var string
	 */
	var $MMFORUMPATH = '';
	...
	function loadLanguageFile($langFile='') {
		if (empty($langFile)) {
			$langFile = 'lang.'.$this->cron_name.'.php';
		}

		$this->langFile = $this->MMFORUMPATH.'cron/lang/'.$langFile;
		if (file_exists($this->langFile)) {
			require_once($this->langFile);
		}
		else {
			$this->debug("Language file not found.", $this->DEBUG_FATAL);
		}
		...
		if (isset($LANG)) {
			...
			$this->debug("Language file could not be read.", $this->DEBUG_FATAL);
		}
	}
	...
	function debug($message, $mode=0) {
		...
		switch($mode) {
			...
			default: if ($this->conf['cron_verbose'] == $this->DEBUGMODE_ALL) {
				echo $message.chr(10);
			} break;
		}
	}
