...
class tx_mmforum_statistics {
	...
	function getStartTime() {
		...
		list($minPostTime) = $this->databaseHandle->sql_fetch_row($res);
		if ($minPostTime == '') {
			return 0;
		}
		return $minPostTime;
...
';
	...
	function stat_forum() {
		...
		list($total_amount) = $this->databaseHandle->sql_fetch_row($res);
		...
		while($ctg = $this->databaseHandle->sql_fetch_assoc($res)) {
			...
			$i ++;
			while($frm = $this->databaseHandle->sql_fetch_assoc($res2)) {

				$res3 = $this->databaseHandle->exec_SELECTquery(
					'COUNT(*)',
					$table,
					"$timeField >= $tstamp_Start AND $timeField <= $tstamp_Stop AND deleted=0 AND forum_id=".$frm['uid']
				);
				list($records) = $this->databaseHandle->sql_fetch_row($res3);
				$ctgRecords += $records;
