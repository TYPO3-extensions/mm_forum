 ...
 *
 *
 *   64: class tx_mmforum_postparser
 *   82:     function main($parent,$conf,$text,$job)
 *  110:     function parse_text($text,$parent,$conf)
 *  136:     function linkgenerator($content,$conf,$switch)
 *  192:     function getMailTo($mailAddress,$linktxt,$initP='?')
 *  221:     function bbcode2html($text,$parent,$conf)
 *  286:     function generate_smilies($text,$parent,$conf)
 *  315:     function validate_email($email)
 *  333:     function syntaxhighlighting($content, $parent, $conf)
 *  384:     function zitat($text,$parent, $conf)
 *  425:     function decode_entities($text)
 *  441:     function links($text, $conf)
 *
 * TOTAL FUNCTIONS: 11
...
class tx_mmforum_postparser {
	 ...
	 */
	function main($parent,$conf,$text,$job){
	...
	function parse_text($text,$parent,$conf){
		...
		if($conf['postparser.']['bb_code_parser'] ==1) 		$content = $this->bbcode2html($content,$parent,$conf);
	...
	function bbcode2html($text,$parent,$conf){  
		...
		while(list($k,$v) = each($buffer)){
			foreach($v[0] as $code_item){$text = preg_replace('/###'.$k.'_BUFFER_MARKER_MMCMS###/',"$code_item", $text, 1);}
		}
	    return $text;
	}
