 ...
 *  787:     function imgtag($imgInfo,$debug=TRUE)
 *  797:     function getLanguage()
 *  816:     function getPidQuery($tables="")
 *  853:     function getFirstPid()
 *  876:     function getAbsUrl($link)
 *  891:     function getIsRealURL()
 *
 * TOTAL FUNCTIONS: 14
 ...
 */
class tx_mmforum_pi3 extends tslib_pibase {
	...
	*/
	function list_inbox ($content,$conf) {
		...
		if($this->piVars['sel_action'] == "archiv") {
			foreach ($this->piVars['messid'] as $value) {
				$value = intval($value);
				$where = 'uid = '.$value.' AND to_uid = '.$GLOBALS['TSFE']->fe_user->user['uid'];
		...
		$where = 'hidden = 0 AND deleted = 0 AND to_uid = '.$GLOBALS['TSFE']->fe_user->user['uid'].' AND mess_type = '.$mess_type.$this->getPidQuery();
		...
		$template = $this->cObj->getSubpart($template, "###MESSAGES_TOP###");
		...
		$template = $this->cObj->fileResource($conf['template.']['main']);
		$template = $this->cObj->getSubpart($template, "###MESSAGES###");
		$marker = array();
		...
		while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
            ...
            // Generate author link
                ...
                elseif($userdata['deleted']=='1')       $marker['###FROM###'] = $this->shield($userdata[tx_mmforum_pi1::getUserNameField()]);
                else {
            ...
            // Generate reply link
                ...
                } else $marker['###PM###']		= tx_mmforum_pi1::createButton('pmreply',$linkParams,0,true,'',true);
		...
		$template = $this->cObj->fileResource($conf['template.']['main']);
		$template = $this->cObj->getSubpart($template, "###MESSAGES_END###");
	...
	function message_del ($content,$conf) {
		$where = 'hidden = 0 AND DELETED = 0 AND uid = \''.intval($this->piVars["messid"]).'\' AND to_uid = '.$GLOBALS['TSFE']->fe_user->user['uid'].$this->getPidQuery();
	...
	*/
	function message_read ($content,$conf) {
		...
		if ($res) {
            ...
            if($this->getIsRealURL()) $linkParams[$this->prefixId]['folder'] = $this->piVars['folder']?$this->piVars['folder']:'inbox';
		...
		elseif ($this->piVars["save"] == $this->pi_getLL('write.send')) {
			...
			// Spam protection: just one message per $conf['block_time']
				// Load last sent message from database
					$where = 'from_uid ='.$GLOBALS['TSFE']->fe_user->user['uid'].' AND mess_type=0 '.$this->getPidQuery();
					$orderBy = 'crdate DESC';
			...
			else
			{
				...
				if ($row) {
					...
					$where = 'uid = '.$row['uid'];
					$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('email, username, tx_mmforum_pmnotifymode','fe_users',$where,'',$orderBy,$limit=1);
					$row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res);
					$to = $row['email'];
					...
					if($row['tx_mmforum_pmnotifymode'] == 0) {
	                    ...
	                    if($this->getIsRealURL()) $linkParams[$this->prefixId]['folder'] = 'inbox';
	...
	*/
	function count_new_pm($uid) {
		...
		$where = 'hidden = 0 AND deleted = 0 AND read_flg = 0 AND mess_type = 0 AND to_uid = '.$GLOBALS['TSFE']->fe_user->user['uid'].$this->getPidQuery();
    ...
    function imgtag($imgInfo,$debug=TRUE){
		return tx_mmforum_pi1::imgtag($imgInfo,$debug);
	}
    ...
    */
     ...
     * See tx_mmforum_pi1::getAbsUrl for more information.
     * 
    ...
    function getAbsUrl($link) {
        return tx_mmforum_pi1::getAbsUrl($link);
    }
    ...
    function getIsRealURL() {
    	return tx_mmforum_pi1::getIsRealURL();
    }
    ...
    function shieldURL($text) {
    	return $this->validatorObj->specialChars_URL($text);
    }	
}
