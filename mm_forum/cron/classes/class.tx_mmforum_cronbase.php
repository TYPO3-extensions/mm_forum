...
class tx_mmforum_cronbase {
	...
	/**
	 * This cronjob's name.
	 * This attribute is meant to be overwritten by the actual cronjob classes.
	 */
	 ...
	 * This function initializes the cronjob object. The initialization of a cronjob
	 * consists of:
	...
	/**
	 * Validates the configuration file and exits with an error if something is missing
	 * @author  Martin Helmich <m.helmich@mittwald.de>
	 * @version 2008-06-22
	 * @return  void
	 */
	 ...
	 * @author  Martin Helmich <m.helmich@mittwald.de>
	 * @version 2008-06-22
	 * @param   string $langFile The name of the language file. If left empty, the default
	 ...
	 * @author  Martin Helmich <m.helmich@mittwald.de>
	 * @version 2008-06-22
	 * @param   string $filename The filename of the template file. This parameter must contain
	 *                           only the filename. The template directory is specified in the
	 *                           configuration file.
	 ...
	 * The date format is specified in the mm_forum configuration file. This function
	 * is capable of working with both the 'date' and the 'strftime' function and
	 * automatically recognizes which of the two functions to use.
	 * 
	 * @author  Martin Helmich <m.helmich@mittwald.de>
	 * @version 2008-06-22
	 ...
	 * @version 2008-06-22
	 * @param   string $key The key of the cache variable.
	 * @return  mixed       The cache variable. Since the cache value is stored in 
	 *                      serialized form, this value can be of any type. If the value
	 *                      was not found in the cache table, this function will return
	 *                      NULL.
	 */
	 ...
	 * This function prints a debug message. The message will be treated differently
	 * depending on the message type specified by $mode. A fatal error will cause the
	 * entire script to stop at once, while other messages will just be displayed or not
	 * in accordance to the settings specified in the configuration file.
	 * 
	 * @author  Martin Helmich <m.helmich@mittwald.de>
	 * @version 2008-06-22
	 * @param   string $message The message to be displayed
	 * @param   int    $mode    The message mode. Determines what is done with the message.
	 * @return  void
	 */
	function debug($message, $mode=0) {
		if($this->conf['cron_verbose'] == $this->DEBUGMODE_QUIET || strlen($message)==0) return;
		switch($mode) {
			case $this->DEBUG_FATAL: die('FATAL ERROR - '.$message.chr(10)); break;
			case $this->DEBUG_ERROR: echo 'ERROR - '.$message.chr(10); break;
