 ...
 *  606:     function getPidQuery($tables="")
 *  643:     function getFirstPid()
 ...
 */
class tx_mmforum_pi6 extends tslib_pibase {
	...
	function getAveragePosts() {
        $res=$GLOBALS['TYPO3_DB']->exec_SELECTquery(
            ...
            'tx_mmforum_posts',
            '1=1'.$this->cObj->enableFields('tx_mmforum_posts').$this->getPidQuery()
        );
	...
	function getTotalTopics() {
		$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
			'COUNT(*)',
			'tx_mmforum_topics',
			'1=1'.$this->cObj->enableFields('tx_mmforum_topics').$this->getPidQuery()
		);   
		$arr = $GLOBALS['TYPO3_DB']->sql_fetch_row($res);
		return $arr[0];
	}
