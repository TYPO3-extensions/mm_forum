 ...
 * 1009:     function getPidQuery($tables="")
 * 1046:     function getFirstPid()
 * 1065:     function getMayRead_forum_query($prefix="")
 * 1092:     function pi_getLL($key)
 *
 * TOTAL FUNCTIONS: 24
 ...
 * @subpackage Search
 */
class tx_mmforum_pi4 extends tslib_pibase {
	...
	function main($content,$conf)	{
		...
		#$param = t3lib_div::GPvar('mmfsearch');
	...
	function searchfind($conf,$searchstring,$param) {
		...
		IF($treffer > 0) {
			...
			foreach($post_id_array as $post_id => $values) {
				...
				$post_text  = strip_tags(tx_mmforum_tools::textCut($post_text,350,''));
				$word_array = explode(" ",$searchstring);
				...
				#$topic_info['topic_title'] = htmlspecialchars($topic_info['topic_title']);
				
				$marker['###TITLE###']      = $this->pi_linkToPage($this->shield($topic_info['topic_title']),$conf['pid_forum'],'',$linkparams);
				$marker['###SHORTTEXT###']  = $this->shield($post_text);
